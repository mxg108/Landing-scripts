...
| REAL `call_states` enum (2026-07-19 subscription audit, engineering's prod sub 4859989923299328): calling, ringing, connected, merged, hold, queued, voicemail, voicemail_uploaded, monitor, barge, hangup, admin, parked, takeover, transcription, recording, csat, missed, recap_summary — plus `dispositions` (dedicated sub 5260350388928512) | live (audit) | subscription setup | design-era `call_transcription` is `transcription`; `dispositions` and `csat` are DEDICATED states — likely the carriers of `call_dispositions` / AI-CSAT payloads (verify shape on first live event). Engineering's comms service already subscribes company-wide to both — coordination point |
