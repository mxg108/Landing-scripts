...
| agent-status events (`target.type == 'user'`, no call_id) | **LIVE 2026-07-29** | webhook_events append + SSE `agent_status` toast (PR #156) | CORRECTION to the C2 synthetic: the live payload carries **no `state` key** (blank '' violated 005's `state_not_blank` CHECK → 500 per delivery until the fold fallback) and its only clock is **`date`** (epoch-ms, now a fold timestamp candidate). Carries `target{id,name,type:'user'}`. The status **value's** key is still unverified — fold tries `new_status`/`status`/`agent_state`/`on_duty_status`, else `'unknown'`; inspect `raw_payload` of a live row and pin the real key here |
