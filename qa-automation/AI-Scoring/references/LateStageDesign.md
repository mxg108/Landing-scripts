...
| ~~Dialpad call disposition → scoring/RAG primer~~ | Owner mandate 2026-07-19 | **Designed → `DispositionDesign.md`** (2026-07-19, slices D1–D4) — struck per registry rule. Evidence overturned the original notes: transcript moments are occurrence MARKERS (no values; the filter is rotted and leaks `[AgentName] at ts` noise into the prompt today); the value source is webhook `call_dispositions` (August) with the Stats API as interim pull + batch backfill |
